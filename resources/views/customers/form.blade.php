...
<form method="POST" action="{{ $customer ? route('customers.update', $customer) : route('customers.store') }}" class="mx-auto max-w-3xl">
    ...
    <section class="card space-y-5">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            <div><label class="label" for="gender">Sexo <span class="font-normal normal-case text-[#777d87]">· opcional</span></label><select class="select" id="gender" name="gender"><option value="">No registrado</option><option value="male" @selected(old('gender', $customer?->gender) === 'male')>Hombre</option><option value="female" @selected(old('gender', $customer?->gender) === 'female')>Mujer</option><option value="non_binary" @selected(old('gender', $customer?->gender) === 'non_binary')>No binario</option><option value="prefer_not_to_say" @selected(old('gender', $customer?->gender) === 'prefer_not_to_say')>Prefiere no indicarlo</option></select><p class="mt-1.5 text-[11px] text-[#777d87]">Se usa solo si el cliente lo declara; nunca se infiere.</p></div>
        </div>
        <div class="rounded-2xl border border-[#d4af37]/15 bg-[#d4af37]/[.035] p-4 sm:p-5">
            <div class="mb-4"><p class="eyebrow">Fechas importantes · opcional</p><p class="mt-1 text-xs leading-5 text-[#858b95]">Permiten recordar al cliente en Inicio y en Celebraciones. No envían mensajes automáticamente.</p></div>
            <div class="grid gap-4 sm:grid-cols-2">
                <div><label class="label" for="birth_date">Cumpleaños</label><input class="input" id="birth_date" name="birth_date" type="date" max="{{ now()->toDateString() }}" value="{{ old('birth_date', $customer?->birth_date?->format('Y-m-d')) }}">@error('birth_date')<p class="field-error">{{ $message }}</p>@enderror</div>
                <div><label class="label" for="anniversary_date">Aniversario</label><input class="input" id="anniversary_date" name="anniversary_date" type="date" max="{{ now()->toDateString() }}" value="{{ old('anniversary_date', $customer?->anniversary_date?->format('Y-m-d')) }}">@error('anniversary_date')<p class="field-error">{{ $message }}</p>@enderror<p class="mt-1.5 text-[11px] text-[#777d87]">Puede ser una fecha personal o importante para el cliente.</p></div>
            </div>
        </div>
        @if($customer)
