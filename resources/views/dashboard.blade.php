    ...
    <div class="card">
        ...
        <div class="mt-4 grid gap-3 sm:grid-cols-3">
            @forelse($recentCampaigns as $campaign)
                <a href="{{ route('campaigns.show', $campaign) }}" class="card-soft card-interactive"><strong class="block truncate text-sm">{{ $campaign->name }}</strong><span class="mt-2 block text-2xl font-black">{{ $campaign->recipients_count }}</span><span class="text-[11px] text-[#858b95]">{{ $campaign->recipients_count === 1 ? 'cliente incluido' : 'clientes incluidos' }} · {{ ['draft' => 'Borrador', 'scheduled' => 'Programada', 'queued' => 'En cola', 'processing' => 'Procesando', 'paused' => 'Pausada', 'completed' => 'Procesada', 'cancelled' => 'Cancelada'][$campaign->status] ?? $campaign->status }}</span></a>
            @empty
